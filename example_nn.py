...
# here I have a dataset with about 3000 rows, where force = sin(velocity)*cos(velocity)+random_number_betw_0_1
my_dataframe = pd.read_csv("data_example.csv", sep=",")
...
def preprocess_targets(my_dataframe):
  ...
  output_targets["force"] = (
    my_dataframe["force"])  # / 1000.0)
  return output_targets

# Choose the first 2000 (out of 3000) examples for training.
training_examples = preprocess_features(my_dataframe.head(2000))
training_targets = preprocess_targets(my_dataframe.head(2000))

# Choose the last 1000 (out of 3000) examples for validation.
validation_examples = preprocess_features(my_dataframe.tail(1000))
validation_targets = preprocess_targets(my_dataframe.tail(1000))
...
my_test_data = pd.read_csv("data_example_test.csv", sep=",")
